...
A ce stade du projet, nous avons dû choisir des extensions que nous implémenterons, la description de celles-ci se trouve dans la suite de ce rapport, dans le section "Extensions".
...
Enfin, nous avons les encore réalisé deux étapes. La première consiste en la création de la base de données proprement dite (fichier Bartender.sqlite) tandis que la seconde est la création d'un liste reprenant toutes les commandes qui nous ont permis de créer notre base de donnée ainsi que d'autres commandes qui nous on permis de tester si celle-ci avait été correctement construite. Cette dernière se trouve dans ce rapport dans la section "Requête SQL".
